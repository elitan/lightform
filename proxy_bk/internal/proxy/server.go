...
func (s *Server) monitorCertificateReloadTrigger() {
	...
	// Check for trigger file changes every 500ms for faster response
		...
		case <-ticker.C:
			...
			if info, err := os.Stat(triggerFile); err == nil {
				...
				if modTime > lastModTime {
					...
					// Process any new domain files first
					s.processNewDomainFiles()
...
// processNewDomainFiles processes domain files and adds them to the certificate manager
func (s *Server) processNewDomainFiles() {
	...
	for _, file := range files {
		if strings.HasPrefix(file.Name(), "luma-proxy-domain-") {
			domainFile := "/tmp/" + file.Name()
			...
			// Parse domain info: hostname|email|timestamp
			parts := strings.Split(string(data), "|")
			if len(parts) >= 1 {
				hostname := parts[0]
				log.Printf("Processing domain file for: %s", hostname)

				// Add domain to certificate manager immediately
				s.certManager.AddDomain(hostname)
				log.Printf("✅ Domain %s immediately added to certificate manager", hostname)

				// Remove the domain file after processing
