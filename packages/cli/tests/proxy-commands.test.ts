...
describe("Proxy Command Argument Parsing Tests", () => {
  ...
  test("should construct correct docker commands", () => {
    const IOP_PROXY_NAME = "iop-proxy";
    
    // Test delete-host command construction
    const deleteHostCmd = `docker exec ${IOP_PROXY_NAME} /usr/local/bin/iop-proxy delete-host api.example.com`;
    ...
    const logsCmd = `docker logs --tail 50 ${IOP_PROXY_NAME}`;
    expect(logsCmd).toBe("docker logs --tail 50 iop-proxy");
    
    const customLogsCmd = `docker logs --tail 100 ${IOP_PROXY_NAME}`;
